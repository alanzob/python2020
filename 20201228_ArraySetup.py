...
while iLoop < numMatches:
    ...
    EloCandy(Candy1[iLoop],Candy2[iLoop],ELOScore[VarietyListID.index(Candy1[iLoop])],ELOScore[VarietyListID.index(Candy2[iLoop])],Winner[iLoop],NumVotes[iLoop])
    iLoop = iLoop+1
    pass

iLoop=0
query = "DROP table if exists brian.dbo.ELOOutput; create Table brian.dbo.ELOOutput (VarietyName varchar(max),ELORank float)"
cursor.execute(query)

while iLoop < 15:
    print (VarietyListName[iLoop] + ": "+ str(round(ELOScore[iLoop],2)))
    query = "Insert into brian.dbo.ELOOutput VALUES ('"+VarietyListName[iLoop]+"',"+str(round(ELOScore[iLoop],2))+")"
    cursor.execute(query)
